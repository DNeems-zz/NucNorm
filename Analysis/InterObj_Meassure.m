...
%keyboard
%Uncomment here for plotting
% Cents=Region_Objs{1,1}.getPixel_List(H.Ana_Channels(1),'Centroid','Microns');
% Cents=vertcat(Cents{:});
% NucPL=Region_Objs{1,1}.getPixel_List(1,'Whole','Microns');
% 
% NN_Pix=Region_Objs{1}.NN_Sim{1};
% 
% 
% figure 
% hold on
% scatter3(NN_Pix(:,1),NN_Pix(:,2),NN_Pix(:,3),'^','g')
% scatter3(NucPL{1}(:,1),NucPL{1}(:,2),NucPL{1}(:,3),'x','k')
% scatter3(Cents(:,1),Cents(:,2),Cents(:,3),'o','r')
